...
| 方法 | 核心作用 | 面试必杀技（怎么说） |
| :--- | :--- | :--- |
| **`then(onFulfilled, onRejected)`** | 注册**成功**和**失败**的回调，**返回新的 Promise**（支持链式）。 | “`then` 的本质是**状态传递**，return 一个普通值会被自动包成 `Promise.resolve`，这也是解决**回调地狱**的关键。” |
| **`catch(onRejected)`** | `then(null, onRejected)` 的语法糖，专门捕获异常。 | “**一定要在链式调用的末尾加 `.catch()`**，否则报错会‘静默消失’，这在 WeLink 的 IM 消息拉取中是我必加的兜底。” |
| **`finally(onFinally)`** | 无论成功或失败，都会执行（状态无关）。 | “用来做**‘结束态’操作**，比如隐藏全局 Loading（加载动画）。注意它不接收任何参数，拿不到结果或错误。” |

###### 2. 静态方法（类上的，用于创建或组合多个 Promise）

| 方法 | 核心作用 | 面试必杀技（怎么说） |
| :--- | :--- | :--- |
| **`Promise.resolve(value)`** | 快速创建一个**已成功**的 Promise。 | “常用于**变量类型统一**，配合 `async/await` 做数据兜底。” |
| **`Promise.reject(error)`** | 快速创建一个**已失败**的 Promise。 | “常用于**异常分支占位**。” |
| **`Promise.all([p1, p2])`** | **全成功才成功，一个失败就失败**（快速失败）。| “适用于**多个互不依赖但需要全部拿到才能渲染的接口**，比如首页的‘用户信息 + 配置信息’并行拉取。**注意**：只要有一个 reject，就不会等其他的了。” |
| **`Promise.allSettled([p1, p2])`** | **全部跑完，返回每个状态的对象**（`{status, value/reason}`）。| “适用于**批量日志上报、资源埋点**，即使某个上报挂了，也不影响其他上报。**这是面试官非常喜欢的健壮性写法！**” |
| **`Promise.race([p1, p2])`** | **谁先完成（成功或失败）就返回谁的结果**。| “**控制请求超时**。把真实请求和 `setTimeout` 赛跑，哪个先完成就返回哪个。” |
| **`Promise.any([p1, p2])`** | **有一个成功就返回成功的值**，全失败才拒绝。（ES2021 新特性）。| “适用于**多个备用 CDN 资源地址**，哪个最快加载成功就用哪个，提升弱网体验。” |
