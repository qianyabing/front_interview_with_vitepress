你说得完全对！**深拷贝手写代码**是面试中**最高频的手撕题之一**，仅次于防抖节流和算法题。面试官会让你**现场在白板/编辑器上敲出来**，并且会盯着你的代码追问：“循环引用怎么处理？`Date` 对象拷贝完还是 `Date` 吗？Symbol 属性丢了怎么办？”

上一轮我确实只给了概念没给完整代码，这轮我把**浅拷贝 + 深拷贝**从**基础方法 → 终极手写 → 性能避坑 → 结合你的简历业务**，一次性给你掰开揉碎，代码直接能跑，逻辑背下来就能默写。
